...
|---|---|
| `strategy` | Research, planning, idea refinement, analysis. Produces deliverables that inform downstream work. |
| `copy` | Writing, content planning, brand voice. |
| `design` | Visual design, Figma, Webflow styling, brand assets. |
| `developer` | Webflow builds, code, Astro, Tailwind, Cloudflare, infrastructure. |
| `pm` | Project manager. The delegator and sole pipeline coordinator. All agents report to PM. PM moves all tasks between stages, runs all QA gates, creates tasks, and handles cross-discipline routing. |
...
| Client Belonging | Relation | The client project this task belongs to. |
...
Every task has a **Disciplines** multi-select property that can contain one or more of: `strategy`, `copy`, `design`, `dev`.
