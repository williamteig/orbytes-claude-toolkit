import { runFleshOut } from './flesh-out';
...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		if (path === '/api/tasks/flesh-out' && request.method === 'POST') {
			return handleFleshOutManual(request, env, ctx);
		}
...
async function handleNotionWebhook(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	const notionPageId = typeof body.notionPageId === 'string' ? body.notionPageId : null;

	if (action === 'flesh_out') {
		if (!notionPageId) {
			return json({ error: 'notionPageId_required' }, 400);
		}
		const pageId = normalizeNotionPageId(notionPageId);
		ctx.waitUntil(
			runFleshOut(env, pageId).catch((err: unknown) => {
				console.error('flesh_out_failed', err);
			}),
		);
		return json({
			ok: true,
			accepted: true,
			mode: 'flesh_out_async',
			notionPageId: pageId,
		});
	}
	...
	const devId = typeof body.devId === 'number' && Number.isFinite(body.devId) ? Math.floor(body.devId) : null;
	...
	await env.DB.prepare(
     ...
     VALUES (?, ?, ?, 'run', 'pending', ?, ?, ?)`,
		...
		.bind(id, devId, notionPageId, now, now, payload)
		.run();

	return json({ ok: true, jobId: id, action: 'run', devId, notionPageId });
}

async function handleFleshOutManual(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
	const secret = env.WEBHOOK_SECRET;
	if (!secret) {
		return json({ error: 'server_misconfigured', detail: 'WEBHOOK_SECRET' }, 500);
	}
	...
	return json({ ok: true, accepted: true, mode: 'flesh_out_async', notionPageId: pageId });
}
...
async function patchJob(request: Request, env: Env, id: string): Promise<Response> {
	...
	const now = new Date().toISOString();
	const result = await env.DB.prepare(`UPDATE jobs SET status = ?, updated_at = ? WHERE id = ? AND status IN ('pending', 'claimed')`)
		.bind(next, now, id)
		.run();
